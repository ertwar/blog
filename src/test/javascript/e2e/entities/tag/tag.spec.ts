...
describe('Tag e2e test', () => {
  ...
  it('should load Tags', async () => {
    ...
    expect(await tagComponentsPage.getTitle()).to.eq('blogApp.tag.home.title');
  ...
  it('should load create Tag page', async () => {
    ...
    expect(await tagUpdatePage.getPageTitle()).to.eq('blogApp.tag.home.createOrEditLabel');
  ...
  it('should create and save Tags', async () => {
    ...
    await promise.all([tagUpdatePage.setTagtextInput('tagtext')]);

    expect(await tagUpdatePage.getTagtextInput()).to.eq('tagtext', 'Expected Tagtext value to be equals to tagtext');
  ...
  it('should delete last Tag', async () => {
    ...
    expect(await tagDeleteDialog.getDialogTitle()).to.eq('blogApp.tag.delete.question');
